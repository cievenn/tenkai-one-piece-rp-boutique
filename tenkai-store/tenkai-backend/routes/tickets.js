...
const { db, generateTicketUid } = require('../database/init');
...
const VALID_CATEGORIES = ['Bug', 'Plainte', 'Boutique', 'Question', 'Autre'];
...
router.get('/', isAuthenticated, (req, res) => {
  try {
    ...
    if (req.user.role === 'Staff' || req.user.role === 'Admin') {
      tickets = db.prepare(`
        SELECT t.*, u.username AS author_name, u.avatar AS author_avatar, u.steam_id AS author_steam_id,
...
router.get('/:uid', isAuthenticated, (req, res) => {
  try {
    ...
    const isOwner = ticket.author_id === req.user.id;
    const isStaff = req.user.role === 'Staff' || req.user.role === 'Admin';
    if (!isOwner && !isStaff) return res.status(403).json({ error: 'Accès refusé.' });

    const messages = db.prepare(`
      SELECT tm.*, u.username AS author_name, u.avatar AS author_avatar, u.role AS author_role
...
router.post('/', isAuthenticated, writeLimiter, sanitizeBody, (req, res) => {
  try {
    ...
    const result = db.prepare(`
      INSERT INTO tickets (ticket_uid, author_id, title, category) VALUES (?, ?, ?, ?)
...
router.post('/:uid/messages', isAuthenticated, writeLimiter, sanitizeBody, (req, res) => {
  try {
    ...
    db.prepare('INSERT INTO ticket_messages (ticket_id, author_id, content) VALUES (?, ?, ?)')
      .run(ticket.id, req.user.id, content);
